const hotelWebsiteDemoUrl = "https://crixus-sarsing-hotel.vercel.app";
...
document.querySelectorAll(".iframe-shell").forEach((shell) => {
  const card = shell.closest(".tour-card");
  const url = card?.dataset.tour;
  if (!url) return;

  const fallback = document.createElement("div");
  fallback.className = "iframe-message";
  fallback.innerHTML = "<p>إذا لم تظهر الجولة داخل الصفحة، افتحها بالحجم الكامل.</p>";
  ...
  open.textContent = "فتح الجولة بحجم كامل";

  setTimeout(() => {
    const card = shell.closest(".tour-card");
    if (card && !card.contains(fallback)) {
...
const hotelDemoLink = document.querySelector("[data-hotel-demo-link]");
if (hotelDemoLink && hotelWebsiteDemoUrl.trim()) {
  hotelDemoLink.href = hotelWebsiteDemoUrl.trim();
  hotelDemoLink.hidden = false;
}

const hotelDemoFrame = document.querySelector("[data-hotel-demo-frame]");
if (hotelDemoFrame && hotelWebsiteDemoUrl.trim()) {
  hotelDemoFrame.src = hotelWebsiteDemoUrl.trim();
}
